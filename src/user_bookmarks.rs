...
pub fn user_bookmarks(base_url: &url::Url, phpsessid: &str, user_id: &str) -> Result<Vec<super::Feed>, String> {
    ...
    if res.status == hyper::status::StatusCode::Ok {
        ...
        for li in doc.find(select::predicate::Class("image-item")) {
            ...
            let thumb_node = li.find(select::predicate::Name("img")).next().expect(
                "Unable to find thumbnail node",
            );
            let thumb = thumb_node.attr("data-src").expect(
                "data-src does not exist in thumbnail node",
            );
            let thumb_url = url::Url::parse(thumb).expect("data-src in thumbnail node is unparsable");
            if let Some(link_node) = li.find(select::predicate::Class("work").and(
